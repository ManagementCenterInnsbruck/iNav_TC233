        ...
        for (unsigned pin = 0; pin < sizeof(ioDefUsedMask[0]) * 8; pin++) {
            if (ioDefUsedMask[port] & (1 << pin)) {
            	...
            	if (port < 17)
            		...
            		ioRec->gpio = (Ifx_P *)(0xF003B000 + (((port-7) % 10) << 8));   // ports are 0x100 apart
            	else if (port < 37)
            		ioRec->gpio = (Ifx_P *)(0xF003C000 + (((port-7) % 10) << 8));   // ports are 0x100 apart
            	else if (port < 47)
            		ioRec->gpio = (Ifx_P *)(0xF003D000 + (((port-7) % 10) << 8));   // ports are 0x100 apart
            	else
