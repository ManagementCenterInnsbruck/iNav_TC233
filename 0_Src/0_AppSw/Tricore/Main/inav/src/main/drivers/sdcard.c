...
#undef SDCARD_SPI_INSTANCE
#endif
#define SDCARD_SPI_INSTANCE spiInstance
...
void sdcard_init(bool useDMA)
{
...
#ifdef AURIX
	spiInstance = spiInstanceByDevice(SDCARD_SPI_DEV);
#endif
	...
	IfxQspi_SpiMaster* master = spiMasterByDevice(SDCARD_SPI_DEV);
    ...
    GPIO_TypeDef* port = IO_GPIO(IOGetByTag(IO_TAG(SDCARD_SPI_CS_PIN)));
	uint8_t pinIndex = IO_Pin(IOGetByTag(IO_TAG(SDCARD_SPI_CS_PIN)));
	...
	IfxQspi_SpiMaster_initChannel(spiInstance, &chConfig);
	spiInstance->activateSlso = NULL;
	spiInstance->deactivateSlso = NULL;
