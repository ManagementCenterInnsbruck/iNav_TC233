...
#ifdef AURIX
static void ppmEdgeCallback(timerCCHandlerRec_t* cbRec, captureCompare_t capture_rise, captureCompare_t capture_fall)
{
	UNUSED(cbRec);
	ppmISREvent(SOURCE_EDGE, capture_rise);

	 int32_t i;

    ppmDev.deltaTime    = capture_rise >> 6;
    ppmDev.overflowed = false;
...
#ifdef AURIX
static void pwmEdgeCallback(timerCCHandlerRec_t *cbRec, captureCompare_t capture_rise, captureCompare_t capture_fall)
#else
...
#ifdef AURIX
    pwmInputPort->capture = capture_fall >> 6;						//64MHz clock
