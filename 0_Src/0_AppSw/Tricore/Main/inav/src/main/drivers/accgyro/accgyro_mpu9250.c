...
#ifdef USE_DUAL_GYRO
    // SET CS of not used MPU
    GPIO_TypeDef* port = IO_GPIO(gyroConfig()->gyro_to_use == 0 ? IOGetByTag(IO_TAG(GYRO_1_CS_PIN)) : IOGetByTag(IO_TAG(GYRO_0_CS_PIN)));
	uint8_t pinIndex = IO_Pin(gyroConfig()->gyro_to_use == 0 ? IOGetByTag(IO_TAG(GYRO_1_CS_PIN)) : IOGetByTag(IO_TAG(GYRO_0_CS_PIN)));
