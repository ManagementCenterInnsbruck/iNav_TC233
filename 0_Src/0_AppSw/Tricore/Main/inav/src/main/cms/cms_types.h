...
#define OSD_LABEL_ENTRY(label)                  { label, OME_Label, {.func = NULL}, NULL, 0 }
#define OSD_LABEL_DATA_ENTRY(label, data)       { label, OME_Label, {.func = NULL}, data, 0 }
#define OSD_LABEL_DATA_DYN_ENTRY(label, data)   { label, OME_Label, {.func = NULL}, data, DYNAMIC }
#define OSD_LABEL_FUNC_DYN_ENTRY(label, fn)     { label, OME_LabelFunc, {.func = NULL}, fn, DYNAMIC }
#define OSD_BACK_ENTRY                          { "BACK", OME_Back, {.func = NULL}, NULL, 0 }
#define OSD_SUBMENU_ENTRY(label, menu)          { label, OME_Submenu, {.func = NULL}, menu, 0 }
#define OSD_FUNC_CALL_ENTRY(label, fn)          { label, OME_Funcall, {.func = fn}, NULL, 0 }
#define OSD_BOOL_ENTRY(label, val)              { label, OME_Bool, {.func = NULL}, val, 0 }
#define OSD_BOOL_CALLBACK_ENTRY(label, cb, val) { label, OME_Bool, {.func = cb}, val, 0 }
#define OSD_BOOL_FUNC_ENTRY(label, fn)          { label, OME_BoolFunc, {.func = NULL}, fn, 0 }
#define OSD_UINT8_ENTRY(label, val)             { label, OME_UINT8, {.func = NULL}, val, 0 }
#define OSD_UINT8_CALLBACK_ENTRY(label, cb, val){ label, OME_UINT8, {.func = cb}, val, 0 }
#define OSD_UINT16_ENTRY(label, val)            { label, OME_UINT16, {.func = NULL}, val, 0 }
#define OSD_UINT16_DYN_ENTRY(label, val)        { label, OME_UINT16, {.func = NULL}, val, DYNAMIC }
#define OSD_UINT16_RO_ENTRY(label, val)         { label, OME_UINT16, {.func = NULL}, val, DYNAMIC | READONLY }
#define OSD_INT16_DYN_ENTRY(label, val)         { label, OME_INT16, {.func = NULL}, val, DYNAMIC }
#define OSD_INT16_RO_ENTRY(label, val)          { label, OME_INT16, {.func = NULL}, val, DYNAMIC | READONLY }
#define OSD_STRING_ENTRY(label, str)            { label, OME_String, {.func = NULL}, str, 0 }
#define OSD_TAB_ENTRY(label, val)               { label, OME_TAB, {.func = NULL}, val, 0 }
#define OSD_TAB_DYN_ENTRY(label, val)           { label, OME_TAB, {.func = NULL}, val, DYNAMIC }
#define OSD_TAB_CALLBACK_ENTRY(label, cb, val)  { label, OME_TAB, {.func = cb}, val, 0 }

#define OSD_END_ENTRY                           { NULL, OME_END, {.func = NULL}, NULL, 0 }
